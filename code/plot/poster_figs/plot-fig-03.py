...
def make_figure_filename():
    ...
    filename = f"fig-03-{catchment}-{forecast_date_range[0]}-{forecast_date_range[-1]}-{suffix}-{REFERENCE_DATASET}-{observation_years[0]}-{observation_years[-1]}.png"
...
def get_monthly_records_excluding_hans(reference_ds, variable):
    ...
    if STORM_HANS_YEAR in selected["year"]:
        hans_mask = (selected["year"] == STORM_HANS_YEAR) & (
            selected["month"] == STORM_HANS_MONTH
        )
...
def make_legend_handles():
    ...
    return [
        ...
        Line2D(
            ...
            markerfacecolor=REFERENCE_COLOR, markeredgecolor=REFERENCE_COLOR,
            markersize=6, label=f"{reference_label} Storm Hans, Aug 2023",
        ),
        Line2D(
...
def plot_monthly_extreme_distributions(
    ...
    reference_records = get_monthly_records_excluding_hans(
