...
2. Equal year-weighted bootstrap
   BALANCE_START_YEAR and BALANCE_END_YEAR define which years may contribute. For
   ...
   If BALANCE_START_YEAR and BALANCE_END_YEAR are both None, all available years are
...
def validate_settings():
    ...
    if BALANCE_START_YEAR is not None and BALANCE_START_YEAR < 1:
        raise ValueError("BALANCE_START_YEAR must be None or a positive year.")
    if BALANCE_END_YEAR is not None and BALANCE_END_YEAR < 1:
        raise ValueError("BALANCE_END_YEAR must be None or a positive year.")
    if (
        BALANCE_START_YEAR is not None
        and BALANCE_END_YEAR is not None
        and BALANCE_END_YEAR < BALANCE_START_YEAR
    ):
...
def read_model_month_by_year(month):
    ...
    if BALANCE_START_YEAR is not None:
        equal_year_mask &= sample_year >= BALANCE_START_YEAR
    if BALANCE_END_YEAR is not None:
        equal_year_mask &= sample_year <= BALANCE_END_YEAR

    equal_year_months = np.unique(sample_month[equal_year_mask])
    if equal_year_months.size == 0:
        start_label = "start of dataset" if BALANCE_START_YEAR is None else BALANCE_START_YEAR
        end_label = "end of dataset" if BALANCE_END_YEAR is None else BALANCE_END_YEAR
        raise ValueError(
    ...
    pooled_mask = equal_year_mask if POOLED_USE_BALANCE_YEARS else calendar_month_mask
...
def print_summary(month_analyses):
    ...
    start_label = "start of dataset" if BALANCE_START_YEAR is None else BALANCE_START_YEAR
    end_label = "end of dataset" if BALANCE_END_YEAR is None else BALANCE_END_YEAR
    print(f"Equal-year years: {start_label}-{end_label}")
