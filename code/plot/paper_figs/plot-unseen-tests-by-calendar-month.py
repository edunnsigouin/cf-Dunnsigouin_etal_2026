...
# Reference years used for fitting and calendar-record calculations.
# Storm Hans is always read separately from August 2023 in the full reference file.
...
def validate_settings():
    ...
    if REFERENCE_DATASET != "senorge":
        raise ValueError("REFERENCE_DATASET must be 'senorge' for the fixed reference file.")
    if CATCHMENT != "regine_drammen":
        raise ValueError("CATCHMENT must be 'regine_drammen' for the fixed reference file.")
    if X_DAYS != 2:
        raise ValueError("X_DAYS must be 2 for the fixed reference file.")
...
def make_reference_filename():
    """Return the fixed full reference file used for fitting and Hans extraction."""

    if REFERENCE_DATASET != "senorge":
        raise ValueError(
            "This script is configured to use the fixed SeNorge reference file."
        )

    return (
        Path(config.dirs["senorge_processed"])
        / "monthly_max_samples_rr_2dayacc_regine_drammen_1957-2025.nc"
    )
...
def read_reference_month(month):
    """Read Hans from the full reference file and fit data from OBSERVATION_YEARS."""
    ...
    if values.size < 10:
        raise ValueError(
            f"Fewer than 10 finite {MONTH_NAMES[month - 1]} values remain "
            f"in OBSERVATION_YEARS."
        )
...
def print_summary(top_results, metric_results):
    ...
    for month in PANEL_MONTHS:
        ...
        record_range = f"{OBSERVATION_YEARS[0]}-{OBSERVATION_YEARS[1]}"
        print()
