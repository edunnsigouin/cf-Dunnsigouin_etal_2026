...
Workflow:
1. Load 2023 yearly seNorge runoff for map panels.
2. Load Drammen catchment weights.
3. Calculate multi-year catchment-weighted runoff time series.
4. Calculate 1958-2022 daily climatology.
5. Plot four runoff maps.
6. Plot 2023 runoff time series with 1958-2022 95% interval and median.
"""
...
CLIMATOLOGY_YEARS = np.arange(1958, 2023)
ALL_TIMESERIES_YEARS = np.arange(1958, 2024)
...
# =============================================================================
# 7. Runoff and weight loading
# =============================================================================

def load_runoff_file(filename, variable):
    """Load one yearly seNorge runoff file."""
...
def load_catchment_weights(filename, weight_var):
    ...
    # Important: runoff y increases, weights y may decrease.
    w = w.sortby("y")

    check_dims(w, ("y", "x"), "Catchment weights")
...
def match_weights_to_runoff_grid(da_runoff, weights):
    ...
    This avoids coordinate-matching problems when the runoff and weight files
    use the same grid but slightly different coordinate values.
    """
...
# =============================================================================
# 8. Catchment-mean runoff calculations
# =============================================================================
...
def calculate_catchment_mean_runoff_for_year(year, weights):
    """Load one runoff year and calculate catchment-mean runoff."""

    filename = PATH_SENORGE_RUNOFF / f"{RUNOFF_VAR}_{year}.nc"

    ds, da_runoff = load_runoff_file(
        filename=filename,
        variable=RUNOFF_VAR,
    )
...
def build_multiyear_catchment_runoff_timeseries(years, weights):
    """Build catchment-mean runoff time series for several years."""

    yearly_series = []

    for year in years:
        print(f"Processing runoff time series for {year}")

        da_year = calculate_catchment_mean_runoff_for_year(
            year=int(year),
            weights=weights,
        )

        yearly_series.append(da_year)

    da_all = xr.concat(yearly_series, dim="time").sortby("time")

    return da_all


def runoff_year_and_climatology_by_calendar_day(
    da_all,
    plot_year,
    climatology_years,
):
    """
    Extract one year's runoff and calendar-day climatology.

    The climatology is calculated from 1958-2022.
    February 29 is removed so all years map cleanly onto a non-leap plot year.
    """

    da_all = da_all.dropna("time")
    ...
    da_plot = da_all.sel(
        time=slice(f"{plot_year}-01-01", f"{plot_year}-12-31")
    )

    da_clim = da_all.sel(
        time=slice(
            f"{climatology_years[0]}-01-01",
            f"{climatology_years[-1]}-12-31",
        )
    )

    da_plot = da_plot.assign_coords(
        calendar_day=da_plot["time"].dt.strftime("%m-%d")
    )

    da_clim = da_clim.assign_coords(
        calendar_day=da_clim["time"].dt.strftime("%m-%d")
    )

    q = da_clim.groupby("calendar_day").quantile(
        [0.025, 0.5, 0.975],
        dim="time",
    )

    calendar_days = da_plot["calendar_day"].values

    q_low = q.sel(quantile=0.025).sel(calendar_day=calendar_days).values
    q_median = q.sel(quantile=0.5).sel(calendar_day=calendar_days).values
    q_high = q.sel(quantile=0.975).sel(calendar_day=calendar_days).values
    ...
    y_year = da_plot.values

    return x_dates, y_year, q_low, q_high, q_median


# =============================================================================
# 9. Load catchment boundary
# =============================================================================
...
def plot_runoff_timeseries(ts_ax, da_all):
    """Plot 2023 runoff with 1958-2022 climatology."""

    x, y, lo, hi, med = runoff_year_and_climatology_by_calendar_day(
        da_all=da_all,
        plot_year=PLOT_YEAR,
    ...
    ts_ax.set_xlim(
        pd.Timestamp(f"{PLOT_YEAR}-01-01"),
        pd.Timestamp(f"{PLOT_YEAR}-12-31"),
    )
...
def main():
    ...
    # 1. Load 2023 runoff data for the map panels.
    ds_runoff_map, da_runoff_map = load_runoff_file(
    ...
    try:
        ...
        # 3. Calculate multi-year catchment-mean runoff time series.
        da_runoff_ts = build_multiyear_catchment_runoff_timeseries(
            years=ALL_TIMESERIES_YEARS,
            weights=weights,
        )
        ...
        mesh = plot_map_panels(
            map_axes=map_axes,
            da_runoff=da_runoff_map,
            catchment_boundary=catchment_boundary,
            proj_data=proj_data,
        )

        # 7. Plot runoff climatology time series.
