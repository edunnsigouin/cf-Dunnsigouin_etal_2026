...
one user-selected variable. Set PLOT_VARIABLE to "precipitation", "msl", or
"snowmelt".

For "snowmelt", the plotted field is daily change in snow water equivalent:
...
# Options: "precipitation", "msl", "snowmelt"
PLOT_VARIABLE = "snowmelt"
...
def get_file_id(catchment_name):
    ...
    return catchment_name.replace("regine_", "", 1) if catchment_name.startswith("regine_") else catchment_name
...
def find_raw_s2s_file(event, variable):
    ...
    candidates = [make_s2s_file(event, variable, grid) for grid in ["0.5x0.5", "0.25x0.25"]]
...
def load_snowmelt(event, target_date):
    """Return daily SWE change; negative values indicate snowmelt."""
...
def load_plot_variable(event, target_date):
    """Load the selected field for one map panel."""
    settings = get_variable_settings(PLOT_VARIABLE)
    if PLOT_VARIABLE == "snowmelt":
        return load_snowmelt(event, target_date)
    return load_daily_variable(event, target_date, settings["raw_variable"])
...
def load_catchment_outer_boundary(filename, base_dir, crs_if_missing="EPSG:4326"):
    ...
    return gpd.GeoDataFrame(geometry=[outer_geom], crs=metric_crs).to_crs("EPSG:4326").geometry.iloc[0]
...
def plot_shaded_field(ax, da, proj_data):
    ...
    if PLOT_VARIABLE == "precipitation":
...
def add_panel_titles(axes, event_dates):
    ...
    for ax, panel_label, date in zip(axes.flat, ["a)", "b)", "c)", "d)"], event_dates):
        ax.set_title(
...
def add_legend(axes, catchment_label):
    ...
    legend = axes[0, 0].legend(handles=[handle], loc="upper left", frameon=True, fontsize=9)
