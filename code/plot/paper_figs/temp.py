...
6. A bottom runoff time-series panel at the grid point nearest Drammen.
"""
...
def make_output_filename(catchment_name, event_rank):
    ...
    return f"{PATH_OUT}fig-0X1-precip-runoff.png"
...
def load_runoff_timeseries_at_drammen(event):
    """Load selected runoff time series at the grid point nearest Drammen."""
    grids_to_try = ["0.5x0.5", "0.25x0.25"]
...
def make_figure_axes():
    """Create four map panels and one full-width runoff time-series panel."""
...
def add_colorbar(fig, mesh):
    """Add horizontal precipitation colorbar below the map panels."""
    cax = fig.add_axes([0.075, 0.31, 0.40, 0.018])
...
def add_legend(fig, catchment_label):
    """Add map legend below the right-hand map panels."""
    legend_handles = [
        Line2D(
            [0],
            [0],
...
def finalize_figure(
    ...
    """Add titles, colorbar, legends, save the figure, and show it."""
