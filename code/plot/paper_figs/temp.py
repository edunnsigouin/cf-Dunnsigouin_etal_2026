...
5. Adds a sixth zoomed panel showing 2-day accumulated precipitation
   for days -1 and 0, with station markers.
"""
...
ZOOM_MAP_EXTENT = [4.1, 14, 57.9, 63.0]#[4.75, 12.75, 58.0, 63.0]
...
def make_output_filename(catchment_name):
    ...
    return (
        ...
        f"{catchment_name}-"
        f"{EVENT_DATES[0]}-{EVENT_DATES[-1]}-with-zoom.png"
    )
...
def make_map_axes(central_lon, central_lat, extent, zoom_extent):
    ...
    axes[1, 2].set_extent(zoom_extent, crs=proj_data)
...
def get_zoom_axis(axes):
    """Return the lower-right zoom panel axis."""
    return axes[1, 2]
...
def plot_zoom_panel(ax, ds_tp, catchment_boundary, target_date, stations, proj_data):
    """Plot zoomed precipitation for day 0."""
    da_precip = load_precipitation(ds_tp, target_date)
...
def add_colorbar(fig, mesh):
    """Add horizontal precipitation colorbar below the left and middle panels."""
    cax = fig.add_axes([0.05, 0.055, 0.59, 0.025])
...
def main():
    ...
    try:
        ...
        mesh = plot_zoom_panel(
            ax=get_zoom_axis(axes),
            ds_tp=ds_tp,
            catchment_boundary=catchment_boundary,
            target_date="2023-08-08",
            stations=STATIONS,
            proj_data=proj_data,
        )    
