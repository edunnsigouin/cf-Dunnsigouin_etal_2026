...
2. Year-balanced repeated sampling
   BALANCE_START_YEAR and BALANCE_END_YEAR define which years may contribute to the
   balanced sample. For the calendar month being analysed, values are kept separate
   by year. The smallest finite realization count among those year-month groups
   defines the common target count. Every replicate samples exactly that many values
   without replacement from each year and then pools the equally represented years.
   This is repeated balanced subsampling rather than a classical bootstrap.

   POOLED_USE_BALANCE_YEARS controls whether the pooled bootstrap uses the same year
   range. If False, the pooled bootstrap uses all available years in the model file.
   If True, it uses only BALANCE_START_YEAR through BALANCE_END_YEAR.
...
# Sampling comparison
# Balanced sampling uses only these years. None means through the latest year present.
BALANCE_START_YEAR = 2004
BALANCE_END_YEAR = None

# False: pooled bootstrap uses every available year in the model file.
# True: pooled bootstrap uses the same year range as balanced sampling.
POOLED_USE_BALANCE_YEARS = True
...
def validate_settings():
    ...
    if BALANCE_START_YEAR < 1:
        raise ValueError("BALANCE_START_YEAR must be a positive year.")
    if BALANCE_END_YEAR is not None and BALANCE_END_YEAR < BALANCE_START_YEAR:
        raise ValueError("BALANCE_END_YEAR must be >= BALANCE_START_YEAR.")
    if not isinstance(POOLED_USE_BALANCE_YEARS, bool):
        raise ValueError("POOLED_USE_BALANCE_YEARS must be True or False.")
...
def read_model_month_by_year(month):
    """Read pooled values and year-balanced groups for one calendar month."""
    ...
    balance_mask = calendar_month_mask & (sample_year >= BALANCE_START_YEAR)
    if BALANCE_END_YEAR is not None:
        balance_mask &= sample_year <= BALANCE_END_YEAR

    balance_months = np.unique(sample_month[balance_mask])
    if balance_months.size == 0:
        end_label = "end of dataset" if BALANCE_END_YEAR is None else str(BALANCE_END_YEAR)
        raise ValueError(
            f"No {MONTH_NAMES[month - 1]} model samples were found from "
            f"{BALANCE_START_YEAR} through {end_label}."
        )

    values_by_yyyymm = {}
    for yyyymm in balance_months:
    ...
    target_count = min(month_values.size for month_values in values_by_yyyymm.values())
    if target_count < 1:
        raise ValueError(f"The year-balanced target count is zero for month {month}.")

    pooled_mask = balance_mask if POOLED_USE_BALANCE_YEARS else calendar_month_mask
    ...
    return {
        ...
        "balance_start_year": int(balance_months.min() // 100),
        "balance_end_year": int(balance_months.max() // 100),
...
def print_summary(month_analyses):
    ...
    balance_end = "end of dataset" if BALANCE_END_YEAR is None else BALANCE_END_YEAR
    print(f"Balanced years:   {BALANCE_START_YEAR}-{balance_end}")
    print(f"Pool same years:  {POOLED_USE_BALANCE_YEARS}")
    ...
    for month in PANEL_MONTHS:
        ...
        print(f"  Balanced years used:           {model['balance_start_year']}-{model['balance_end_year']}")
        print(f"  Number of balanced years:      {len(counts)}")
