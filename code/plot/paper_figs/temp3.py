...
RANGE_FILL_COLOR = "0.75"
RANGE_FILL_ALPHA = 0.5
...
def make_output_filename(catchment_name, event_rank):
    ...
    return f"{PATH_OUT}fig-0X1-runoff-drammen-timeseries-range.png"
...
def load_runoff_hindcast_member_range_at_drammen(event):
    """
    Load min-max runoff envelope at the grid point nearest Drammen.

    The envelope is computed from all hindcast dates and ensemble members
    for the same forecast initialization date, excluding the selected
    hdate/member event.
    """
    if event["model_type"] != "hindcast":
        raise ValueError(
            "The hindcast/member range is only defined for hindcast events."
        )
    ...
    for grid in grids_to_try:
        ...
        try:
            ...
            da_min.attrs["selected_grid"] = grid
            da_min.attrs["selected_lon"] = float(da_point[lon.name].values)
            da_min.attrs["selected_lat"] = float(da_point[lat.name].values)
            da_min.attrs["n_samples_total"] = n_samples_total
            da_min.attrs["n_samples_used"] = n_samples_used

            da_max.attrs.update(da_min.attrs)
    ...
    raise FileNotFoundError(
        f"Could not find hindcast/member range file "
        f"for {event['forecast_date']}."
    )
...
def plot_runoff_timeseries(ts_ax, event, event_dates):
    """Plot selected runoff time series and hindcast/member min-max envelope."""
    da_event = load_runoff_timeseries_at_drammen(event)
    da_min, da_max = load_runoff_hindcast_member_range_at_drammen(event)
