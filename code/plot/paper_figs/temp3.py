...
def calculate_log_likelihood(
    ...
    This is the log-likelihood of the observed values under the fitted model.
    """
    ...
    if EXTREME_VALUE_DISTRIBUTION == 1:
        ...
        # genex_negative_log_likelihood() returns -log(L).
        # Convert the fitted positive parameters back to log-parameters
        # because that is the form expected by the function.
        log_pdf_sum = -genex_negative_log_likelihood(
            np.log(
                fitted_parameters
            ),
            values,
...
def get_number_of_fitted_parameters():
    ...
    # Gumbel and the GenEx implementation used here each have two parameters.
    return 2
...
def calculate_aic(
    ...
    when comparing models fitted to exactly the same observations.
    """
...
def print_fit_summary(
    ...
    print(
        f"Parameters, k:    {number_of_parameters}"
    )
    print(
