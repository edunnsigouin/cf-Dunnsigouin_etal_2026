...
def print_selected_event(event):
    ...
    raw_member = get_raw_member_label(event)
    if raw_member != event["ensemble_member"]:
        print(f"  raw_number_label : {raw_member}")
...
def get_raw_member_label(event):
    """Return the ensemble label used in the original raw S2S file."""
    member = int(event["ensemble_member"])
    if event["model_type"] == "hindcast" and member == 11:
        return 51
    return member
