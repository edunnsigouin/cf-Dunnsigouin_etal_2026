...
The script first creates one pooled box-and-whisker distribution containing
all finite samples across the selected lead times. It then plots one
distribution for every available lead time. For a 1-day accumulation, the
expected lead times are 16 through 46.
...
def prepare_boxplot_values(data):
    """
    Return the pooled distribution followed by one distribution per lead time.

    The first boxplot contains all finite samples pooled across every selected
    lead time. The remaining boxplots contain the individual lead-time
    distributions.
    """
    ...
    pooled_values = np.concatenate(values_by_lead)

    plot_labels = np.concatenate(
        (
            np.asarray(["All"], dtype=object),
            lead_times.astype(str),
        )
    )
    ...
    return lead_times, plot_labels, plot_values
...
def print_distribution_summary(
    ...
    """Print summary statistics for the pooled and lead-time distributions."""
    ...
    print(
        f"All leads: "
        f"n={pooled_values.size:6d}, "
        f"median={np.median(pooled_values):8.3f}, "
        f"q25={np.percentile(pooled_values, 25):8.3f}, "
        f"q75={np.percentile(pooled_values, 75):8.3f}"
    )
