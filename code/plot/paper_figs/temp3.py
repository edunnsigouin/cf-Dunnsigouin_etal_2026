...
Panel e shows the catchment-mean evolution of that same variable for every ensemble
member in the event file, with the selected event member highlighted in dark blue.
For "sm":
...
TIMESERIES_MEMBER_COLOR = "0.75"
TIMESERIES_MEMBER_LINEWIDTH = 0.8
TIMESERIES_MEMBER_ALPHA = 0.8
...
def load_ensemble_timeseries(event, catchment_name):
    """Load catchment means for all members of the selected event file."""
    ...
    for grid in ["0.5x0.5", "0.25x0.25"]:
        ...
        try:
            da = select_event_hdate(ds[raw_variable], event)
            ...
            if PLOT_VARIABLE == "sm":
                ...
                da_mean.name = "sm"
                da_mean.attrs.update(
                    units="mm",
                    long_name="Daily change in snow water equivalent",
                )

            da_mean = da_mean.load()
            selected_member = get_raw_member_label(da_mean, event)
            da_mean.attrs["selected_grid"] = grid
            return da_mean, selected_member
        finally:
            ds.close()
...
def plot_ensemble_timeseries(ax, event, catchment_name):
    """Plot panel e: catchment-mean evolution for all ensemble members."""
    da, selected_member = load_ensemble_timeseries(event, catchment_name)
    time_name = get_time_coord_name(da)
    member_name = get_member_coord_name(da)
    times = da[time_name].values

    for member in da[member_name].values:
        ax.plot(
            times,
            da.sel({member_name: member}).values,
            color=TIMESERIES_MEMBER_COLOR,
            linewidth=TIMESERIES_MEMBER_LINEWIDTH,
            alpha=TIMESERIES_MEMBER_ALPHA,
            zorder=1,
        )

    selected = da.sel({member_name: selected_member})
    ax.plot(
        times,
        selected.values,
        color=TIMESERIES_SELECTED_COLOR,
