...
Each panel contains:
    ...
    - areas with snow water equivalent greater than zero (SWE > 0),
      shown as stippling or hatching;
...
pressure, and snow-covered areas evolve around one extreme S2S event.
"""
...
SWE_OVERLAY = "stippling"  # options: "stippling", "hatching"
SWE_THRESHOLD = 0.0
SWE_DOT_COLOR = "orange"
SWE_DOT_SIZE = 5
SWE_DOT_ALPHA = 1.0
SWE_DOT_STRIDE = 1
SWE_HATCH_PATTERN = "////"
SWE_HATCH_COLOR = "orange"
SWE_HATCH_LINEWIDTH = 1.0
SWE_ZORDER = 10
...
def make_output_filename(catchment_name, event_rank):
    ...
    figname = f"{PATH_OUT}fig-05-{FORECAST_DATE_RANGE[0]}-{FORECAST_DATE_RANGE[-1]}-{CATCHMENT_NAME}-month-{EVENT_MONTH}-rank-{EVENT_RANK}.png"
    return figname
...
def load_swe(event, target_date):
    """Load snow water equivalent for one plotted date in mm."""
    da_swe = load_daily_variable(event, target_date, SNOW_VAR)
    da_swe.attrs["units"] = "mm"
    da_swe.attrs["long_name"] = "Snow water equivalent"
    return da_swe
...
def plot_swe(ax, da_swe, proj_data):
    """Overlay locations where snow water equivalent is greater than zero."""
    lon, lat = get_lon_lat(da_swe)
    swe_mask = np.isfinite(da_swe.values) & (da_swe.values > SWE_THRESHOLD)
    if SWE_OVERLAY == "stippling":
        plot_swe_stippling(ax, lon, lat, swe_mask, proj_data)
    elif SWE_OVERLAY == "hatching":
        plot_swe_hatching(ax, lon, lat, swe_mask, proj_data)
    else:
        raise ValueError("SWE_OVERLAY must be either 'stippling' or 'hatching'.")

def plot_swe_stippling(ax, lon, lat, swe_mask, proj_data):
    """Plot areas with SWE > 0 as orange stippling."""
    iy, ix = np.where(swe_mask)
    if SWE_DOT_STRIDE > 1:
        iy = iy[::SWE_DOT_STRIDE]
        ix = ix[::SWE_DOT_STRIDE]
    ax.scatter(
        lon.values[ix],
        lat.values[iy],
        s=SWE_DOT_SIZE,
...
def plot_swe_hatching(ax, lon, lat, swe_mask, proj_data):
    """Plot areas with SWE > 0 as orange hatching."""
    old_hatch_color = plt.rcParams["hatch.color"]
    old_hatch_linewidth = plt.rcParams["hatch.linewidth"]
    try:
        plt.rcParams["hatch.color"] = SWE_HATCH_COLOR
        plt.rcParams["hatch.linewidth"] = SWE_HATCH_LINEWIDTH
        ax.contourf(
            lon.values,
            lat.values,
            swe_mask.astype(int),
...
def get_swe_legend_handle():
    """Return legend handle for the selected SWE overlay."""
    if SWE_OVERLAY == "hatching":
        return Patch(
            facecolor="white",
            edgecolor=SWE_HATCH_COLOR,
            hatch=SWE_HATCH_PATTERN,
            label="Snow cover (SWE > 0)",
        )
    return Line2D(
        [0],
        [0],
