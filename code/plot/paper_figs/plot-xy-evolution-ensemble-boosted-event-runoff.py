#!/usr/bin/env python3
"""
Plot 24-hour accumulated runoff for one S2S forecast ensemble member.

The figure contains:
1. Four map panels showing ro24 for one selected forecast ensemble member.
2. A bottom panel, e), showing Drammen catchment-spatial-mean ro24:
...
OUTPUT_FILENAME = f"{PATH_OUT}xy_runoff_maps_ensemble_boosting_event_with_forecast_era5_timeseries.png"
...
RUNOFF_LEVELS = np.arange(2, 22, 2)
...
def get_catchment_settings(catchment_name):
    """Return filename, weights ID, and label for the selected catchment."""
...
def make_s2s_file(variable):
    """Return the expected S2S NetCDF file path."""
    return (
        S2S_BASE_DIR
        / MODEL_TYPE
        / "sfc"
...
def get_member_coord_name(da):
    """Return the ensemble member coordinate name used by a DataArray."""
...
def standardize_runoff_units(da):
    ...
    S2S and ERA5 accumulated runoff variables are commonly stored in metres.
    """
...
# =============================================================================
# 7. Forecast runoff loading
# =============================================================================

def open_forecast_runoff_dataset():
    """Open forecast ro24 and convert to mm/day."""
...
def load_runoff_map(target_date):
    """Load ro24 for the selected ensemble member and target date."""
...
def load_all_forecast_runoff_members():
    """Load ro24 for all forecast ensemble members."""
    ds = open_forecast_runoff_dataset()
...
def load_era5_runoff(years, time_start, time_end):
    """Load ERA5 ro24 over the requested period."""
    filenames = [
...
def load_forecast_catchment_mean(catchment_name, init_date, plot_end):
    """Load catchment-mean forecast ro24 for all ensemble members."""
...
def load_era5_catchment_mean(catchment_name, years, load_start, load_end,
                             plot_start, plot_end):
    """Load catchment-mean ERA5 ro24 for the full panel-e date window."""
    spatial_dims = ("latitude", "longitude")
...
def make_figure_axes():
    ...
    return fig, axes, ts_ax, cbar_ax, proj_data
...
# =============================================================================
# 12. Map plotting functions
# =============================================================================

def plot_runoff_map(ax, da_runoff, proj_data):
    """Plot runoff as shaded grid cells."""
...
def plot_event_panel(ax, target_date, catchment_boundary, proj_data):
    ...
    da_runoff = load_runoff_map(target_date)
...
# =============================================================================
# 13. Time-series plotting
# =============================================================================

def plot_catchment_mean_timeseries(ts_ax, catchment_name):
    """Plot forecast ensemble and ERA5 catchment-mean ro24 in panel e)."""
    (
        init_date,
        plot_start,
        plot_end,
    ...
    # Plot all non-selected ensemble members.
    ...
    # Highlight selected member used in panels a-d.
    ...
    # Mark the four map dates on the selected member.
...
def add_colorbar(fig, mesh, cbar_ax):
    ...
    cbar.set_label(
        "24-hour accumulated runoff (mm/day)",
...
def finalize_figure(
    ...
    """Add titles, colorbar, legends, time series, save if requested, and show."""
    add_panel_titles(get_plot_axes(axes))
...
def main():
    ...
    fig, axes, ts_ax, cbar_ax, proj_data = make_figure_axes()
