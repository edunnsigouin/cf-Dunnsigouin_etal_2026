...
The horizontal dashed reference line can show either the August 2023 Storm
Hans maximum or the highest reference-dataset value for SELECTED_MONTH during
1957-2022.
...
# Choose which observational event to show as the horizontal dashed line.
#
# Options:
#     "storm_hans" -> August 2023 maximum from the reference dataset
#     "record"     -> highest value for SELECTED_MONTH during 1957-2022
#
HORIZONTAL_LINE_EVENT = "record"
...
def validate_user_settings():
    ...
    if HORIZONTAL_LINE_EVENT not in {
        "storm_hans",
        "record",
    }:
        raise ValueError(
            "HORIZONTAL_LINE_EVENT must be 'storm_hans' or 'record'."
        )
...
def get_horizontal_line_event(
    storm_hans_value,
    record_value,
    record_year,
):
    """
    Return the value and legend label for the selected horizontal reference line.
    """

    if HORIZONTAL_LINE_EVENT == "storm_hans":

        if storm_hans_value is None:
            raise ValueError(
                "HORIZONTAL_LINE_EVENT is 'storm_hans', but no finite "
                "August 2023 value was found in the reference dataset."
            )
    ...
    if record_value is None:
        raise ValueError(
            "HORIZONTAL_LINE_EVENT is 'record', but no finite "
            f"{RECORD_START_YEAR}-{RECORD_END_YEAR} record was found "
            "for SELECTED_MONTH."
        )

    return (
        record_value,
        f"Record {RECORD_START_YEAR}-{RECORD_END_YEAR}",
    )
...
def plot_return_period_curves(
    ...
    # -------------------------------------------------------------------------
    # Selected observational reference event
    # -------------------------------------------------------------------------
    ...
    # Keep the legend deliberately simple: one entry per dataset.
    legend_handles = [
        Line2D(
            [0],
            [0],
...
def main():
    ...
    print(
        f"Horizontal line:               "
        f"{reference_line_label}"
    )

    print(
        f"Horizontal-line value:         "
        f"{reference_line_value:.3f} mm"
    )

    if HORIZONTAL_LINE_EVENT == "record":
        print(
            f"Record year:                   "
            f"{record_year}"
        )
    ...
    reference_line_return_period_reference = (
        calculate_event_return_period(
            event_value=reference_line_value,
    ...
    reference_line_return_period_raw = (
        calculate_event_return_period(
            event_value=reference_line_value,
    ...
    print()
    print(
        f"{reference_line_label} value: "
        f"{reference_line_value:.3f} mm"
    )

    print(
        f"{reference_line_label} return period from "
        f"{get_reference_label()} fit: "
        f"{reference_line_return_period_reference:.2f} years"
    )

    print(
        f"{reference_line_label} return period from "
        f"raw UNSEEN fit: "
        f"{reference_line_return_period_raw:.2f} years"
    )

    if bias_corrected_unseen_analysis is not None:

        reference_line_return_period_bc = (
            calculate_event_return_period(
                event_value=reference_line_value,
        ...
        print(
            ...
            f"{reference_line_return_period_bc:.2f} years"
        )
