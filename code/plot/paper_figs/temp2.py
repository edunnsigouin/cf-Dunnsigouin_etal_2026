...
def count_months_within(
    reference_lists: dict[str, list[np.ndarray]],
    s2s_lists: dict[str, list[np.ndarray]],
    statistic,
    n_samples: int,
) -> tuple[
    dict[str, np.ndarray],
    dict[str, dict[str, list[dict[str, float | int | bool]]]],
]:
    """
    Count passing months and retain month-level bootstrap diagnostics.

    The diagnostics contain the reference statistic, bootstrap interval,
    and pass/fail result for every correction method and reference series.
    """
...
def print_raw_failures(
    ...
    reference_name: str = "Reference w/o Hans",
) -> None:
    """Print calendar months that fail the fidelity test for raw S2S data."""
    ...
    print()
    print(
        f"Raw-data failures for {statistic_name}"
    )
    print(
...
def calculate_fidelity(reference: xr.Dataset, s2s: xr.Dataset,
    ...
    for stat_name, statistic in STATISTICS.items():
        ...
        print_raw_failures(
            ...
            reference_name="Reference w/o Hans",
        )
