...
Layout note
-----------
A thin empty row is added between the top precipitation panel and panels a/b.
This increases only that specific vertical separation without increasing the
spacing everywhere else in the figure.
"""
...
OUTPUT_FILENAME = f"{PATH_OUT}fig-0X_combined_precip_maps_runoff.png"
...
def make_figure_axes():
    ...
    - Row 2: map panels a and b
    - Row 3: map panels c and d
    - Row 4: bottom runoff time-series panel e

    The empty spacer row increases only the separation between the top panel
    and panels a/b.
    """
    ...
    # It creates extra vertical space only between the top panel and panels a/b.

    map_axes = np.empty((2, 2), dtype=object)
    map_axes[0, 0] = fig.add_subplot(gs[2, 0], projection=proj_map)
    map_axes[0, 1] = fig.add_subplot(gs[2, 1], projection=proj_map)
    map_axes[1, 0] = fig.add_subplot(gs[3, 0], projection=proj_map)
    map_axes[1, 1] = fig.add_subplot(gs[3, 1], projection=proj_map)

    cbar_ax = fig.add_subplot(gs[2:4, 2])

    runoff_ts_ax = fig.add_subplot(gs[4, 0:2])
