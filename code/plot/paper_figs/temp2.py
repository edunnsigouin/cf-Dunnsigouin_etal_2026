...
SNOW_OVERLAY_VARIABLE = "snowmelt"  # options: "snowmelt", "snow_depth"
...
def load_snow_overlay(event, lag, target_date):
    """Load the snow field used for the selected overlay."""
    if SNOW_OVERLAY_VARIABLE == "snow_depth":
        return load_daily_variable(event, target_date, SNOW_VAR)
    ...
    previous = load_daily_variable(event, str(previous_date), SNOW_VAR)
    current = load_daily_variable(event, str(current_date), SNOW_VAR)
...
def plot_event_panel(ax, event, lag, target_date, catchment_boundary, proj_data):
    ...
    da_snow = load_snow_overlay(event, lag, target_date)
