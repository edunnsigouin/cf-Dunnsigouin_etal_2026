...
2. Year-balanced repeated sampling
   The balancing period is explicitly controlled by BALANCE_START_MONTH and
   BALANCE_END_MONTH. Within that period, the same calendar month is kept separate
   by YYYYMM. The smallest finite sample count among those YYYYMM groups defines a
   common target count. For every
   ...
   each year is without replacement. The pooled bootstrap is not restricted by the
   balance window and continues to use the complete available model dataset.
...
# Applied only to the year-balanced sampling procedure. Use YYYYMM integers.
BALANCE_START_MONTH = 202301
BALANCE_END_MONTH = None  # Set to None to use all later available months.
...
def validate_settings():
    ...
    if BALANCE_START_MONTH < 100001 or BALANCE_START_MONTH > 999912:
        raise ValueError("BALANCE_START_MONTH must be a YYYYMM integer.")
    if BALANCE_START_MONTH % 100 not in range(1, 13):
        raise ValueError("BALANCE_START_MONTH has an invalid calendar month.")
    if BALANCE_END_MONTH is not None:
        if BALANCE_END_MONTH < BALANCE_START_MONTH:
            raise ValueError("BALANCE_END_MONTH must be >= BALANCE_START_MONTH.")
        if BALANCE_END_MONTH % 100 not in range(1, 13):
            raise ValueError("BALANCE_END_MONTH has an invalid calendar month.")
...
def read_model_month_by_year(month):
    """Read pooled values and balance-window YYYYMM groups for one calendar month."""
    ...
    # Pooled bootstrap: use every available realization for this calendar month.
    pooled_mask = sample_month % 100 == month
    ...
    # Year-balanced sampling: use only YYYYMM groups inside the requested balance window.
    balance_mask = pooled_mask & (sample_month >= BALANCE_START_MONTH)
    if BALANCE_END_MONTH is not None:
        balance_mask &= sample_month <= BALANCE_END_MONTH

    balance_months = np.unique(sample_month[balance_mask])
    if balance_months.size == 0:
        end_label = "end of dataset" if BALANCE_END_MONTH is None else str(BALANCE_END_MONTH)
        raise ValueError(
            f"No calendar-month {month:02d} samples were found between "
            f"{BALANCE_START_MONTH} and {end_label}."
        )
    ...
    return {
        ...
        "balance_start_month": int(balance_months.min()),
        "balance_end_month": int(balance_months.max()),
    }
...
def print_summary(month_analyses):
    ...
    balance_end = "end of dataset" if BALANCE_END_MONTH is None else BALANCE_END_MONTH
    print(f"Balance window:   {BALANCE_START_MONTH}-{balance_end}")
    ...
    for month in PANEL_MONTHS:
        ...
        print(f"  Balanced YYYYMM groups:        {len(counts)}")
        print(
            f"  Actual balance period:          "
            f"{model['balance_start_month']}-{model['balance_end_month']}"
        )
