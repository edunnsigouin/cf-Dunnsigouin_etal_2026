...
VARIABLE_SETTINGS = {
    ...
    "ro24": {
        "cmap": "Blues",
        "vmin": RUNOFF_LEVELS[0],
        "vmax": RUNOFF_LEVELS[-1],
        "levels": RUNOFF_LEVELS,
...
def load_ensemble_timeseries(event, catchment_name):
    ...
    for grid in ["0.5x0.5", "0.25x0.25"]:
        ...
        try:
            ...
            if event["model_type"] == "hindcast":
                for name in ["hdate", "hindcast_date"]:
                    if name in da_mean.dims or name in da_mean.coords:
                        hdate_name = name
                        break
            ...
            lower.attrs["selected_grid"] = grid
            upper.attrs["selected_grid"] = grid
...
def plot_ensemble_timeseries(ax, event, event_dates, catchment_name):
    ...
    ax.fill_between(
        ...
        lower.values,
        upper.values,
        color=TIMESERIES_RANGE_COLOR,
        alpha=TIMESERIES_RANGE_ALPHA,
        linewidth=0,
        label=f"95% interval over all hdates and members (n={n_samples})",
        zorder=1,
    )
...
def align_timeseries_axis(fig, axes, timeseries_ax):
    """Align panel e with map width and place it close below panels c-d."""
    fig.canvas.draw()
    left = min(axes[0, 0].get_position().x0, axes[1, 0].get_position().x0)
    right = max(axes[0, 1].get_position().x1, axes[1, 1].get_position().x1)
    map_bottom = min(axes[1, 0].get_position().y0, axes[1, 1].get_position().y0)
    pos = timeseries_ax.get_position()
    height = pos.height
    top = map_bottom - 0.035
    timeseries_ax.set_position([left, top - height, right - left, height])
