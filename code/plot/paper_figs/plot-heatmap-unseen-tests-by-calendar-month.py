...
Rows are the seven model-data options: raw, mm_1step, mm_2step, q, doy, ld, and
q_doy. The first five columns are fixed: independence plus fidelity of the mean,
...
def build_output_filename() -> Path:
    ...
    correction_mode = "fixed-failed-months" if BIAS_CORRECT_ONLY_FAILED_MONTHS else "fixed-all-months"
    return Path(config.dirs["fig"]) / (
        f"Heatmap_UNSEEN_tests_with_{PANEL_F_TEST}_{correction_mode}_"
        f"{x_days}dayacc_{catchment}_{forecast_date_range[0]}_{forecast_date_range[1]}_{REFERENCE_DATASET}_"
...
def validate_user_settings() -> None:
    ...
    if set(BIAS_CORRECTION_METHODS) != valid_methods or len(BIAS_CORRECTION_METHODS) != 6:
        raise ValueError(
            "BIAS_CORRECTION_METHODS must contain each of "
            f"{sorted(valid_methods)} exactly once."
        )
