...
Panels (c) and (d) use the compact monthly-maximum sample file. Panel (c) shows
the number of Hindcast, Forecast, and All realizations assigned to each YYYYMM.
Panel (d) aggregates the same realization counts by calendar month across all
years.
...
forecast_title = "b) Model sampling example for August"
...
map_title_x = -0.20  # Shift title left so it begins above the colorbar.
map_cmap = "BuGn"
map_vmin = 0.0
map_vmax = 1.0
...
def plot_forecast_panel(axis, accumulated):
    ...
    axis.set_xlabel("Valid date", fontsize=axis_label_fontsize)
    axis.set_ylabel(
...
def plot_monthly_count_panel(axis, plot_dates, monthly_counts):
    """Plot realization counts for each YYYYMM in panel (c)."""
    ...
    axis.set_title("c) Realizations by year and month", loc="left", fontsize=title_fontsize, pad=8)
    axis.set_xlabel(
        "Time [year]",
        fontsize=axis_label_fontsize,
    )
...
def plot_calendar_month_count_panel(axis, calendar_month_counts):
    """Plot realization counts aggregated by calendar month in panel (d)."""
    ...
    axis.set_title("d) Realization by month", loc="left", fontsize=title_fontsize, pad=8)
...
def make_figure(
    ...
    monthly_axis = figure.add_subplot(grid[1, 0])
    calendar_month_axis = figure.add_subplot(grid[1, 1])
