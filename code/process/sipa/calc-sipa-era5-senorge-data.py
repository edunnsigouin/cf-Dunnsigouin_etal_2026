...
Output:
- Filename:
  sipa_preprocessed_s2s_drammen_<firstyear>-<lastyear>.nc
- One variable: tp24(date)
- One scalar coordinate: number
- Units: mm
- Values are X-day accumulated catchment-mean precipitation.
"""
...
# Scalar coordinate retained for compatibility with the original
# sipa_preprocessed_era5_drammen.nc structure.
number_value = 0

# Output directory. The filename is generated automatically as:
# sipa_preprocessed_s2s_drammen_<firstyear>-<lastyear>.nc
output_directory = config.dirs["sipa_processed"]
...
def format_like_sipa_preprocessed_era5(
    ...
    """
    Format the output like sipa_preprocessed_era5_drammen.nc.
    ...
    if "time" not in da_out.dims:
        raise ValueError(
            "Expected the accumulated time series to have a 'time' dimension. "
            f"Found dimensions: {da_out.dims}"
        )
    ...
    tp24.attrs["units"] = "mm"
    tp24.attrs["description"] = (
        f"{x_days}-day accumulated catchment-weighted "
        "mean precipitation"
    )

    # Make the scalar number coordinate appear as an auxiliary coordinate
    # of tp24, matching the original file structure.
    tp24.encoding["coordinates"] = "number"
...
def write_output(
    ...
    """Create the ERA5-style output dataset and optionally write it."""

    out = format_like_sipa_preprocessed_era5(
        da_out=da_out,
