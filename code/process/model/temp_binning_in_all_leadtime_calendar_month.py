...
5. Determine the calendar month from the COMPLETE usable lead-time window.
6. Assign the maxima from the complete window and every split lead-time window
   to that same calendar month.

Lead-time splitting
-------------------
The usable accumulated lead-time window is split into N consecutive,
approximately equal bins. The split is performed after accounting for the
X-day accumulation. If the number of usable accumulated lead times is not
divisible by N, the extra lead times are assigned to the later bins.
...
The usable 2-day accumulated ending leads are 17-46 (30 lead times), which
are split equally as:
    17-31
    32-46

The stored maximum variables are therefore:
    max_value_lead17_46   # complete usable lead window
    max_value_lead17_31   # first split
    max_value_lead32_46   # second split

The calendar month is determined ONCE from the complete usable accumulated
lead-time window. The complete-window maximum and all split-window maxima from
that initialization are then assigned to that same calendar month. Thus, the
split samples inherit the calendar-month assignment of the full lead-time
window. This keeps the early and late samples aligned to the same nominal
monthly UNSEEN sample and gives the same number of stored samples in each lead
group for a given month (apart from any missing values).
...
# Number of approximately equal lead-time subsets after x_days accumulation
number_of_lead_bins = 2
...
def build_accumulated_lead_ranges() -> list[tuple[int, int]]:
    """
    Return the full usable accumulated range followed by N equal lead bins.
...
def extract_max_values(
    ...
    Calendar-month assignment is handled separately from the complete usable
    lead window, so this function returns only the flattened maximum values.
    """
...
def get_full_range_month(
    da: xr.DataArray,
    full_range: tuple[int, int],
) -> int:
    """
    Determine the calendar month from the complete usable lead-time window.

    This month is used for the complete-window maximum and for every split
    maximum from the same initialization.
    """

    full_start, full_end = full_range

    selected_full = select_lead_range(
        da=da,
        lead_start=full_start,
        lead_end=full_end,
    )

    return get_month_with_most_lead_dates(selected_full)
    ...
    store.attrs.update(
        {
            "description": (
                ...
                "All split samples inherit the calendar-month assignment of "
                "the complete usable lead-time window."
            ),
            "variable": variable,
            "catchment": catchment,
            "x_days": x_days,
            ...
            "calendar_month_binning": "full usable lead window",
...
def process_and_store_all_ranges(
    ...
    The month is determined once from the complete usable lead-time window and
    then reused for every split. This is the "full-window month" binning
    option.
    """

    full_range = lead_ranges[0]
    month = get_full_range_month(
        da=da,
...
def print_monthly_sample_counts(
    ...
    """Print the number of finite stored samples for each month and lead group."""
    ...
    variable_names = [
        lead_range_variable_name(start, end)
        for start, end in lead_ranges
    ]

    labels = [
        "full" if i == 0 else f"split_{i}"
        for i in range(len(lead_ranges))
    ]
    ...
    for month, month_name in enumerate(month_names, start=1):
        ...
        for variable_name in variable_names:
            values = store[variable_name].sel(month_of_year=month).values
            counts.append(int(np.isfinite(values).sum()))

        row = f"{month_name:<12}" + "".join(
            f"{count:>12d}" for count in counts
        )
        print(row)

    totals = []
    for variable_name in variable_names:
        totals.append(int(np.isfinite(store[variable_name].values).sum()))

    print("-" * len(header))
    print(
        f"{'TOTAL':<12}"
        + "".join(f"{count:>12d}" for count in totals)
    )
